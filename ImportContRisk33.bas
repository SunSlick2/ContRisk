Sub ImportMultipleTSVAndStackDatav7() ' Updated version number
    ...
    Dim cashTradeCollection As Collection ' New: For Cash Trade rows (columns S-T-U)
    ...
    With fd
        ...
        If .Show = -1 Then
            ...
            If lastRow > 1 Then
                ws.Range("B4:F" & lastRow).ClearContents
                ws.Range("L4:W" & lastRow).ClearContents
            End If
            ...
            Do While currentRow <= lastRow
                ...
                If Len(clientIDValue) > 0 And fileMap.Exists(clientIDValue) Then
                                        ...
                                        If i + 1 <= UBound(rows) Then
                                            ...
                                            If InStr(nextRow, "Cash Trades") > 0 Then
                                                ...
                                                If UBound(lineData) >= 5 Then
                                                    ...
                                                    ' Cash Trade amount (usually in same column as Total exposure)
                                                    Dim cashExposure As String
                                                    cashExposure = Trim(lineData(6)) ' Assuming amount is in same column
                    ...
                    If dataCollection.Count = 0 Then
                        ReDim totalData(1 To 1, 1 To 21) ' Now 21 columns (A-U)
                    ...
                    For i = 1 To UBound(totalData, 1)
                        ...
                        ' Initialize Synth. Exp. column (column E) - will be populated from Section D
                        totalData(i, 5) = 0
                        ...
                        totalData(i, 7) = ""
                        ' Columns H-J are for ToFlipTable reference only, not populated here
                        ' Columns K-N are blank
                        ' Column R is blank
                        ' Column V is blank
                    Next i
                    ...
                    ElseIf singleCurrencySuppressed Then
                        ' Single Currency Netting Suppressed - we will handle Column E later
                        ' For now, leave Column E as is (will be overwritten below)
                    ...
                    If singleCurrencySuppressed Then
                        ' Overwrite Column E with values from Column U (SCN Delta in Base Amt)
                        For i = 1 To UBound(totalData, 1)
                        ...
                        ' Add message rows below the client's data
                        ' First, we need to expand the totalData array to accommodate the message rows
                        Dim expandedData() As Variant
                        ReDim expandedData(1 To UBound(totalData, 1) + 2, 1 To 21)
                        ...
                        ' Add message rows
                        ' Row UBound(totalData,1)+1: "SCN suppressed" in column A
                        expandedData(UBound(totalData, 1) - 1, 1) = "SCN suppressed"
                        ' All other columns blank for this row
                        
                        ' Row UBound(totalData,1)+2: "Synth Pos col has cash delta in base ccy" in column A
                        expandedData(UBound(totalData, 1), 1) = "Synth Pos col has cash delta in base ccy"
                        ' All other columns blank for this row
                        
                        ' Replace totalData with expandedData
...
WriteData:
                    ...
                    ' Clear old content in columns A:W if the new data is shorter
