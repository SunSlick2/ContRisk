            ...
            For Each filePath In .SelectedItems
                ...
                If dataCollection.Count > 0 Then
                    ...
                    For i = 1 To dataCollection.Count
                        ' Populate new columns first
                        totalData(i, 1) = clientID
                        totalData(i, 2) = coverRatio
                        
                        For j = 0 To 2
                            ' Populate existing columns
                            If j = 0 Then
                                totalData(i, 3) = dataCollection(i)(j) ' CcyPair
                            ElseIf j = 1 Then
                                totalData(i, 5) = dataCollection(i)(j) ' Risk Ccy
                            ElseIf j = 2 Then
                                totalData(i, 6) = dataCollection(i)(j) ' Exposure (RiskCcy)
                            End If
                        Next j
                ...
                If fxRates.Count > 0 Then
                    ws.Cells(lastRow - (UBound(totalData, 1) + 1), 9).Resize(1, 2).Value = Array("Currency", "Mid Spot Rate")
                    ws.Cells(lastRow - (UBound(totalData, 1) + 1), 9).Resize(1, 2).Font.Bold = False
                    ...
                    For Each key In fxRates.Keys
                        ...
                        i = i + 1
                    Next key
                    ws.Cells(lastRow - UBound(totalData, 1), 9).Resize(UBound(fxData, 1), UBound(fxData, 2)).Value = fxData
