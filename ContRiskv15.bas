...
Sub ImportMultipleTSVAndStackData()
    ...
    With fd
        ...
        If .Show = -1 Then
            ...
            k = 1
            Do While Not IsEmpty(ws.Cells(k, 13).Value) Or Not IsEmpty(ws.Cells(k, 14).Value) Or Not IsEmpty(ws.Cells(k, 15).Value)
                If Not IsEmpty(ws.Cells(k, 13).Value) And Not IsEmpty(ws.Cells(k, 14).Value) And Not IsEmpty(ws.Cells(k, 15).Value) Then
                ...
                If dataCollection.Count > 0 Then
                    ...
                    For i = 1 To dataCollection.Count
                        ...
                        For k = 1 To ToFlipTable.Count
                            Dim flipEntry As Variant
                            flipEntry = ToFlipTable(k)
                            
                            If ccyPair = flipEntry(0) Then
                                ...
                                divCurrency = flipEntry(1) ' Divisor currency from ToFlipTable
                                mulCurrency = flipEntry(2) ' Multiplier currency from ToFlipTable
