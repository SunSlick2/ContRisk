Sub ImportTSVAndWriteData()
    ...
    For i = 0 To UBound(rows)
        ...
        If startSCNRates And Not endSCNRates Then
            ' Check for "FX.Rate." and ".Spot" anywhere in the line, case-insensitive
            If InStr(1, rowData, "FX.Rate.", vbTextCompare) > 0 And InStr(1, rowData, ".Spot", vbTextCompare) > 0 Then
                Dim lineData As Variant
                lineData = Split(rowData, vbTab)
                
                Dim ccy As String
                ' Extract currency by finding the position of "FX.Rate." and ".Spot"
                ccy = Split(lineData(0), ".")(2) ' Assumes format is always FX.Rate.XXX.Spot
