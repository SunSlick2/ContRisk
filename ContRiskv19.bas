    ...
    With fd
        ...
        If .Show = -1 Then
            ...
            For Each filePath In .SelectedItems
                ...
                ' Write Client ID and Cover Ratio once per file
                ...
                ws.Cells(lastRow + 1, 3).Value = Int(coverRatio)
                
                lastRow = lastRow + 3 ' Move to the start of the main data table
                Dim startRowOfMainTable As Long
                startRowOfMainTable = lastRow
                
                ' Write "Total" row data block
                If dataCollection.Count > 0 Then
                    ws.Cells(lastRow, 1).Resize(1, 5).Value = Array("CcyPair", "Risk Ccy", "Exposure(RiskCcy)", "Manipulated Exposure", "Exposure(mio)")
                    
                    ReDim totalData(1 To dataCollection.Count, 1 To 5)
                    For i = 1 To dataCollection.Count
                        ' Populate columns based on the new layout
                        totalData(i, 1) = dataCollection(i)(0) ' CcyPair
                        totalData(i, 2) = dataCollection(i)(1) ' Risk Ccy
                        totalData(i, 3) = dataCollection(i)(2) ' Exposure(RiskCcy)
                        
                    ' Apply manipulation logic if ccypair appears in ToFlipTable
                    ccyPair = totalData(i, 1) ' Use the value from the new array
                    ...
                    For k = 1 To ToFlipTable.Count
                        ...
                        If ccyPair = currentFlipEntry(0) Then
                            ...
                            originalExposure = totalData(i, 3) ' Already a number
                    ...
                    If Not IsEmpty(totalData(i, 4)) And IsNumeric(totalData(i, 4)) Then
                        valueForMio = CDbl(totalData(i, 4))
                    ElseIf IsNumeric(totalData(i, 3)) Then
                        valueForMio = CDbl(totalData(i, 3))
                    Else
                        valueForMio = 0
                    End If
                ...
                If fxRates.Count > 0 Then
                    ws.Cells(startRowOfMainTable, 7).Resize(1, 2).Value = Array("Currency", "Mid Spot Rate")
                    ...
                    For Each key In fxRates.Keys
                        ...
                        i = i + 1
                    Next key
                    ws.Cells(startRowOfMainTable + 1, 7).Resize(UBound(fxData, 1), UBound(fxData, 2)).Value = fxData
