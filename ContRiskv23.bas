    ...
    With fd
        ...
        If .Show = -1 Then
            ...
            For Each filePath In .SelectedItems
                ...
                ' Always write the ClientID and CoverRatio
                ws.Cells(lastRow, 1).Value = clientID
                ws.Cells(lastRow, 2).Value = Int(coverRatio)
                lastRow = lastRow + 1
                
                ' Write "Total" row data block if data exists
                If dataCollection.Count > 0 Then
                    
                    ReDim totalData(1 To dataCollection.Count, 1 To 8)
                    For i = 1 To dataCollection.Count
                        ' The first two columns are already written, so we populate from column 3
                    ...
                    For k = 1 To ToFlipTable.Count
                        ...
                        If ccyPair = currentFlipEntry(0) Then
                            ...
                            originalExposure = totalData(i, 7) ' Already a number
                ...
                Next i
                    ...
                    ws.Cells(lastRow, 3).Resize(UBound(totalData, 1), UBound(totalData, 2) - 2).Value = totalData
                    rowsInMainTable = UBound(totalData, 1) ' No header row to count
                Else
                    rowsInMainTable = 0
                End If
                ...
                If fxRates.Count > 0 Then
                    ws.Cells(startRowOfMainTable, 10).Resize(1, 2).Value = Array("Currency", "Mid Spot Rate")
                    ...
                    For Each key In fxRates.Keys
                        ...
                        i = i + 1
                    Next key
                    ws.Cells(startRowOfMainTable + 1, 10).Resize(UBound(fxData, 1), UBound(fxData, 2)).Value = fxData
