    ...
    With fd
        ...
        If .Show = -1 Then
            ...
            For Each filePath In .SelectedItems
                ...
                startRowForFile = startRowForFile + 3 ' Leave a gap
                
                ' Write "Total" row data block
                If dataCollection.Count > 0 Then
                    ws.Cells(startRowForFile, 1).Resize(1, 3).Value = Array("CcyPair", "RiskCCy", "Exposure (RiskCCy)")
                    ws.Cells(startRowForFile, 1).Font.Bold = True
                    ...
                    For i = 1 To dataCollection.Count
                        ...
                        Next j
                    Next i
                    ws.Cells(startRowForFile + 1, 1).Resize(UBound(totalData, 1), UBound(totalData, 2)).Value = totalData
                    startRowForFile = startRowForFile + UBound(totalData, 1) + 2
                Else
                    startRowForFile = startRowForFile + 2
                End If
                
                ' Write FX rate data block
                If fxRates.Count > 0 Then
                    ws.Cells(startRowForFile, 1).Resize(1, 2).Value = Array("Currency", "Mid Spot Rate")
                    ws.Cells(startRowForFile, 1).Font.Bold = True
                    ...
                    For Each key In fxRates.Keys
                        ...
                        i = i + 1
                    Next key
                    ws.Cells(startRowForFile + 1, 1).Resize(UBound(fxData, 1), UBound(fxData, 2)).Value = fxData
                End If
