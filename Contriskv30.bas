...
Sub ImportMultipleTSVAndStackData()
    ...
    With fd
        ...
        If .Show = -1 Then
            ' Set the output worksheet to "ContRisk2" (creating it if it doesn't exist)
            ...
            ' Clear output area and set initial row
            ws.Range("A:L").Clear
            lastRow = 1
            
            ' Write headers once at the top
            ws.Cells(lastRow, 1).Resize(1, 8).Value = Array("ClientID", "CoverRatio", "CcyPair", "Exposure(mio)", "", "Risk Ccy", "Exposure(RiskCcy)", "Manipulated Exposure")
            ws.Cells(lastRow, 10).Resize(1, 2).Value = Array("Currency", "Mid Spot Rate")
            lastRow = lastRow + 1
            ...
            For Each filePath In .SelectedItems
                ...
                For i = 0 To UBound(rows)
                    If Len(Trim(rows(i))) > 0 Then
                        ...
                        ' We no longer look for "Client:" inside the file.
                    ...
                    If dataCollection.Count > 0 Then
                        ...
                        For k = 1 To ToFlipTable.Count
                            ...
                            If ccyPair = currentFlipEntry(0) Then
                                ...
                                ' DISPLAY FLIP (Requirement #2) - Flip CcyPair for display ONLY
                ...
                ' Write data block
                ws.Cells(lastRow, 1).Resize(UBound(totalData, 1), UBound(totalData, 2)).Value = totalData
                rowsInMainTable = UBound(totalData, 1)
                ...
                If fxRates.Count > 0 Then
                    ...
                    ws.Cells(lastRow, 10).Resize(UBound(fxData, 1), UBound(fxData, 2)).Value = fxData
                    rowsInFxTable = UBound(fxData, 1)
                Else
                    rowsInFxTable = 0
                End If
                
                ' Update lastRow for the next file, ensuring minimum of 1 empty row separation
                lastRow = lastRow + Application.Max(rowsInMainTable, rowsInFxTable) + 1
    ...
    MsgBox "Data import and writing complete to sheet 'ContRisk2'!", vbInformation
