    ...
    With fd
        ...
        If .Show = -1 Then
            ...
            For Each filePath In .SelectedItems
                ...
                ' Add the new rounded down Cover Ratio figure in column C
                ws.Cells(lastRow + 1, 3).Value = Int(coverRatio)
                lastRow = lastRow + 3
                
                ' Write "Total" row data block
                If dataCollection.Count > 0 Then
                    ws.Cells(lastRow, 1).Resize(1, 5).Value = Array("CcyPair", "RiskCCy", "Exposure (RiskCcy)", "Manipulated Exposure", "Exposure (mio)")
                    ...
                    For k = 1 To ToFlipTable.Count
                        ...
                        If ccyPair = currentFlipEntry(0) Then
                            ...
                            If IsNumeric(dataCollection(i)(2)) Then
                                originalExposure = CDbl(dataCollection(i)(2))
                            Else
                                originalExposure = 0 ' Default to 0 if not a number
                            End If
                    ...
                    ' Populate the new "Exposure (mio)" column
                    If IsNumeric(totalData(i, 4)) Then
