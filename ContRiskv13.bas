    ...
    With fd
        ...
        If .Show = -1 Then
            ...
            For Each filePath In .SelectedItems
                ...
                ws.Cells(lastRow, 2).Font.Bold = True ' Bold the Client ID number, not the label
                ...
                If dataCollection.Count > 0 Then
                    ws.Cells(lastRow, 1).Resize(1, 4).Value = Array("CcyPair", "RiskCCy", "Exposure (RiskCCy)", "Manipulated Exposure")
                    ws.Cells(lastRow, 1).Resize(1, 3).Font.Bold = False ' Remove bold from headers
                    
                    Dim totalData() As Variant
                    ReDim totalData(1 To dataCollection.Count, 1 To 4)
                    For i = 1 To dataCollection.Count
                        For j = 0 To 2
                            totalData(i, j + 1) = dataCollection(i)(j)
                        Next j
                        
                        ' Apply manipulation logic for ALL currency pairs in column M (starting from M2)
                        Dim ccyPair As String
                        ccyPair = dataCollection(i)(0)
                        
                        ' Loop through all entries in column M (starting from row 2)
                        k = 2 ' Start from row 2 in column M
                        Do While Not IsEmpty(ws.Cells(k, 13).Value) ' Column M is column 13
                            If ccyPair = ws.Cells(k, 13).Value Then
                                If Not IsEmpty(ws.Cells(k, 14).Value) And Not IsEmpty(ws.Cells(k, 15).Value) Then
                                    ...
                                    divCurrency = ws.Cells(k, 14).Value ' Column N
                                    mulCurrency = ws.Cells(k, 15).Value ' Column O
                                    originalExposure = dataCollection(i)(2)
                                    ...
                                    If divRate <> 0 And mulRate <> 0 Then
                                        ...
                                        ' If multiple manipulations apply, they will overwrite each other
                                        totalData(i, 4) = manipulatedExposure
                                    End If
                                End If
                            End If
                            k = k + 1
                ...
                If fxRates.Count > 0 Then
                    ...
                    ws.Cells(lastRow, 1).Resize(1, 2).Font.Bold = False ' Remove bold from headers
